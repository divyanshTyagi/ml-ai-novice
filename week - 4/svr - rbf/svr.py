...
#linear - best we can observe is  0.3679584281819649 (we are removing the date column , i tried encoding it but mse value was still worse)
#rbf - best we can observe is mse -> .282249799 (2,3,5,6)
...
def plotRegressorTrain(regressor,Y,Y_pred):
    ...
    plt.xlabel('Features')
    plt.ylabel('Price')
    plt.savefig('Feature -  SVR - Train',dpi = 500)
    plt.show()
...
def plotRegressorTest(regressor,Y,Y_pred):
    ...
    plt.ylabel('Price')
    plt.xlabel('Features')
    plt.savefig('Feature - '  + ' SVR - Test',dpi = 500)
    plt.show()




X = dataset.iloc[:,[2,3,5,6]].values
